...
public class MSTestTransformerTest extends TestHelper{
    ...
    public void testReturnWhenNoTRXFileisFound() throws Exception {
        ...
        transformer = new MSTestTransformer("build.trx", converter, buildListener, false);
    ...
    @Test
    public void testIgnoreTRXFileParameter() throws Exception {
        classContext.checking(new Expectations() {
            {
                ignoring(buildListener).getLogger();
                will(returnValue(new PrintStream(new ByteArrayOutputStream())));
        ...
        transformer = new MSTestTransformer("build.trx", converter, buildListener, true);
        Boolean result = transformer.invoke(parentFile, virtualChannel);
        assertFalse("The archiver did not return true when it could not find any files", !result);
    }
}
